...
``pyproject.toml`` reads ``__version__`` from here at build time, so the number
...
it -- and ``tests/test_paper.py`` reads it as text. Keep it free of imports and
of any computed value: a plain literal assignment is what both of them can see,
and it is what keeps ``import pyvoa`` cheap, since ``pyvoa/__init__.py``
re-exports ``__version__`` without pulling in pandas or geopandas.

Releasing: bump ``__version__`` here and nowhere else, then follow the checklist
in CONTRIBUTING.md section 9. The value must be a valid PEP 440 version.
